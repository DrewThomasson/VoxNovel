...
#fuck
#this will import styleTTS2 if its not pip installed already
import subprocess
import sys
...
styleTTS_Model = None
styleTTS_Model_loaded = False
styleTTS = STYLETTS.StyleTTS2()
def generate_style(text, refrence_audio, output):
    styleTTS.inference(text,refrence_audio, output)
def generate_audio():
    # Get device
    global multi_voice_model_voice_list1
    global multi_voice_model_voice_list2
    ...
    for index, row in data.iterrows():
        ...
        temp_count =0
        for sentence in sentences:
            global styleTTS_Model
            global styleTTS_Model_loaded
            ...
            for fragment in fragments:
                ...
                #If the voice actor has a custom fine tuned xtts model in its refrence folder ie if it has the model folder containing it and the selected model is a xtts model
                ...
                # If the selected model is StyleTTS2
                elif "StyleTTS2" in selected_tts_model:
                    # No paths provided means default checkpoints/configs will be downloaded/cached.
                    #tts = None
                    #tts = tts.StyleTTS2()
                    #is the model hasnt been loaded
                    #if styleTTS_Model_loaded != True:
                    #    styleTTS = STYLETTS.StyleTTS2()
                    #    styleTTS_Model_loaded = True
                    #if voice_actor_var != voice_actor:
                    #    tts = None
                    #    tts = STYLETTS.StyleTTS2()
                    #    voice_actor_var = voice_actor
                    # Specify target voice to clone. When no target voice is provided, a default voice will be used.
                    #styleTTS = STYLETTS.StyleTTS2()
                    audio_filz = list_reference_files(voice_actor)
                    audio_file_ref = audio_filz[0]
                    generate_style(fragment, audio_file_ref, f"Working_files/temp/{temp_count}.wav")
                    #styleTTS.inference(fragment, target_voice_path=audio_filz[0], output_wav_file=f"Working_files/temp/{temp_count}.wav")
